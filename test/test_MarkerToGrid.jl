...
x_grid_min = 3.0; dx = 1.4;
y_grid_min = 4.5; dy = 2.0;
...
x_grid_min = 0.0; dx = 0.3;
y_grid_min = 0.0; dy = 0.4;
lx = (Nx-1)*dx;
ly = (Ny-1)*dy;
...
grid2_glob = if rank == 0 zeros(Tuple(dims[1:2]).*size(val_grid2)) else zeros(0,0) end
grid3_glob = if rank == 0 zeros(Tuple(dims[1:2]).*size(val_grid3)) else zeros(0,0) end
