...
@views function interpolateV(x, y, Vx, Vy, x_vx, y_vx, x_vy, y_vy, dx, dy)
    ...
    if ix < 2
        println("WARNING: Vx-interpolation, ix=$(ix) too small")
        ix = 1
        dxij = 0.0
    end
    if iy < 1
        println("WARNING: Vx-interpolation, iy=$(iy) too small")
        iy = 1
        dyij = 0.0
    end
    if ix >= size(Vx, 1) - 1
        println("WARNING: Vx-interpolation, ix=$(ix) too big")
        ix = size(Vx, 1) - 2
        dxij = 1.0
    end
    if iy >= size(Vx, 2)
        println("WARNING: Vx-interpolation, iy=$(iy) too big")
        iy = size(Vx, 2) - 1
        dyij = 1.0
    end
    ...
    #index range failsafe (that should never be used)
    if ix < 1
        println("WARNING: Vy-interpolation, ix=$(ix) too small")
        ix = 1
        dxij = 0.0
    end
    if iy < 2
        println("WARNING: Vy-interpolation, iy=$(iy) too small")
        iy = 1
        dyij = 0.0
    end
    if ix >= size(Vy, 1)
        println("WARNING: Vy-interpolation, ix=$(ix) too big")
        ix = size(Vy, 1) - 1
        dxij = 1.0
    end
    if iy >= size(Vy, 2) - 1
        println("WARNING: Vy-interpolation, iy=$(iy) too big")
        iy = size(Vy, 2) - 2
        dyij = 1.0
    end
    # bilinear Interpolation
